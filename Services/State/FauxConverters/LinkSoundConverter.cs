...
public class LinkSoundConverter(ILogger logger, ISoundPlayer soundPlayer, IPlayniteEventHandler playniteEventHandler)
    : ILinkSoundConverter
{
    protected readonly IPlayniteEventHandler _playniteEventHandler = playniteEventHandler;
        ...
        int endingIndex)
    {
        ...
        /* When */ if (Enum.TryParse<UIState>(uiStateStr, true, out var uiState))
                ...
                /* Then */ switch (linkMethod)
            {
                ...
                case "Initialized":
                    (element as FrameworkElement).Initialized += (_, _) => Trigger(uiState, soundType); return;
                case "TargetUpdated":
                    (element as FrameworkElement).TargetUpdated += (_, _) => Trigger(uiState, soundType); return;
                case "SourceUpdated":
                    (element as FrameworkElement).SourceUpdated += (_, _) => Trigger(uiState, soundType); return;
                case "SelectionChanged":
                    (element as ComboBox).SelectionChanged += (_, _) => Trigger(uiState, soundType); return;
                case "DragCompleted":
                    (element as Thumb).DragCompleted += (_, _) => Trigger(uiState, soundType); return;
                default: return;
            }
            else /* Then */ switch (linkMethod)
            {
                ...
                case "Initialized":
                    (element as FrameworkElement).Initialized += 
                        (_, _) => _playniteEventHandler.TriggerUIStateChanged(uiState); return;
                case "TargetUpdated":
    ...
    private void HandleRoutedEvent(
        ...
        => element.AddHandler(routedEvent, new RoutedEventHandler((_, _) => Trigger(uiState, soundType)));

    private void HandleRoutedEvent(UIElement element, RoutedEvent routedEvent, UIState uiState)
        => element.AddHandler(
            routedEvent, new RoutedEventHandler((_, _) => _playniteEventHandler.TriggerUIStateChanged(uiState)));

    private void Trigger(UIState uiState, SoundType soundType)
    {
        _playniteEventHandler.TriggerUIStateChanged(uiState);
        soundPlayer.Trigger(soundType);
    }

    protected void LogWarning(string messagePrefix, Type type)
        => logger.Warn($"{messagePrefix} while attempting to link '{type.Name}' for LinkSoundConverter");
