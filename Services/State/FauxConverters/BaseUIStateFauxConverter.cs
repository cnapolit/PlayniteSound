...
public abstract class BaseUIStateFauxConverter<T>(ILogger logger, IPlayniteEventHandler playniteEventHandler)
    : IValueConverter
{
    ...
    protected void LogWarning(string messagePrefix) 
        => logger.Warn($"{messagePrefix} while attempting to link '{typeof(T)}' for IValueConverter '{GetType()}'");
