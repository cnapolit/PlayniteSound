...
internal class SpDownloader(ILogger logger, IAssemblyResolver assemblyResolver, HttpClient client)
    : BaseDownloader(logger, client), IDownloader
{
    private const string BaseUrl = "https://open.spotify.com/";

    private readonly SpotifyClient     _client = new(client);
    ...
    private Album AlbumResultToAlbum(AlbumSearchResult result)
    {
        ...
        // ReSharper disable once ConstantNullCoalescingCondition it has been null before
        result.Tracks ??= [];
    ...
    {
        using (assemblyResolver.HandleAssemblies(typeof(IAsyncDisposable)))
    ...
    {
        using (assemblyResolver.HandleAssemblies(typeof(System.Text.Encodings.Web.HtmlEncoder), typeof(Memory<>)))
        {
    ...
    {
        string url;
        using (assemblyResolver.HandleAssemblies(typeof(HtmlAgilityPack.HtmlAttribute))) /* Then */ try
            {
