...
{
    internal class SCDownloader : BaseDownloader, IDownloader
    {
        ...
        public SCDownloader(ILogger logger, IAssemblyResolver assemblyResolver, HttpClient client)
            : base(logger, client)
        {
            _assemblyResolver = assemblyResolver;
            _client = new SoundCloudClient(client);
        }
        ...
        private async Task<Stream> GetStreamAsync(Song song, CancellationToken token)
        {
            ...
            /* Then */ return await CopyToMemoryAsync(stream, token);
        }
        ...
        private IDisposable CreateAssemblyHandler() 
            => _assemblyResolver.HandleAssemblies(typeof(Unsafe), typeof(System.Text.Encodings.Web.HtmlEncoder));
